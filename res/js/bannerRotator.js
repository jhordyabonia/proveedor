  ...
  if(count > 1){ //dont do anything if there is only one list item.  
    ...
    if(nav != null){
      ...
      while(i <= count){
		  ...
		  var texto = "";
			if(i == 1) texto = "Instant Video";
			else if(i == 2) texto = "Appstore For Android";
			else if(i == 3) texto = "Smartphone";
			else if(i == 4) texto = "Laptops";
			else if(i == 5) texto = "Tablets";
			else  texto = "Mp3 Store";
			
	      if(i == $(selector+" li:visible").attr('rel')){  //if its the nav item that belongs to the visible image, mark it as the active nav item
          $('#bannerNav').append("<a class='active' rel='"+i+"' href='#'><span>"+texto +"</span></a> ");
	      } 
	      else{
	        $('#bannerNav').append("<a rel='"+i+"' href='#'><span>"+texto +"</span></a> ");
	      }
	      i++;
   	  }
